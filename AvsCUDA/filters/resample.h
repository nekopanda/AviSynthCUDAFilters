...
// CUDA Resize function pointer
typedef void (*DevResampler)(
...
struct DevResamplingProgram {
	std::unique_ptr<DeviceLocalData<int>> pixel_offset;
	std::unique_ptr<DeviceLocalData<float>> pixel_coefficient;
};
...
private:
  ...
  ResamplerV resampler_chroma;
	...
	DevResamplingProgram dev_program_luma;
	DevResamplingProgram dev_program_chroma;
	DevResampler dev_resampler;
