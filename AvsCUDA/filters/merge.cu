...
__device__ float4 dev_merge(float4 a, float4 b, float w, float invw) { return a * invw + b * w; }
__device__ int4 dev_merge(uchar4 a, uchar4 b, int w, int invw) { return (to_int(a) * invw + to_int(b) * w + 16384) >> 15; }
__device__ int4 dev_merge(ushort4 a, ushort4 b, int w, int invw) { return (to_int(a) * invw + to_int(b) * w + 16384) >> 15; }

template <typename vpixel_t, typename W>
__global__ void kl_merge_plane(vpixel_t* srcp, const vpixel_t* otherp, 
  int src_pitch, int other_pitch, int width, int height,
  W weight, W invweight)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;

  if (x < width && y < height) {
    auto tmp = dev_merge(srcp[x + y * src_pitch], otherp[x + y * other_pitch], weight, invweight);
    srcp[x + y * src_pitch] = VHelper<vpixel_t>::cast_to(tmp);
  }
}
  ...
  float weight, int pixelsize, int bits_per_pixel, PNeoEnv env)
{
  ...
  int src_pitch4 = src_pitch >> (pixelshift + 2);
  int other_pitch4 = other_pitch >> (pixelshift + 2);

  dim3 threads(32, 8);
  dim3 blocks(nblocks(width4, threads.x), nblocks(src_height, threads.y));

  int weight_i = (int)(weight * 32767.0f + 0.5f);
  int invweight_i = 32767 - weight_i;
  float invweight = 1.0f - weight;

  if (pixelsize == 1) {
    kl_merge_plane << <blocks, threads >> >((uchar4*)srcp, (const uchar4*)otherp,
      src_pitch4, other_pitch4, width4, src_height, weight_i, invweight_i);
    DEBUG_SYNC;
  }
  else if (pixelsize == 2) {
    kl_merge_plane << <blocks, threads >> >((ushort4*)srcp, (const ushort4*)otherp,
      src_pitch4, other_pitch4, width4, src_height, weight_i, invweight_i);
    DEBUG_SYNC;
  }
  else { // pixelsize == 4
    kl_merge_plane << <blocks, threads >> >((float4*)srcp, (const float4*)otherp,
      src_pitch4, other_pitch4, width4, src_height, weight, invweight);
    DEBUG_SYNC;
  }
}

__device__ float4 dev_average(float4 a, float4 b) { return (a + b) * 0.5f; }
__device__ int4 dev_average(uchar4 a, uchar4 b) { return (to_int(a) + to_int(b) + 1) >> 1; }
__device__ int4 dev_average(ushort4 a, ushort4 b) { return (to_int(a) + to_int(b) + 1) >> 1; }

template <typename vpixel_t>
__global__ void kl_average_plane(vpixel_t* srcp, const vpixel_t* otherp,
  int src_pitch, int other_pitch, int width, int height)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
  ...
  if (x < width && y < height) {
    auto tmp = dev_average(srcp[x + y * src_pitch], otherp[x + y * other_pitch]);
    srcp[x + y * src_pitch] = VHelper<vpixel_t>::cast_to(tmp);
  }
}
  ...
  int pixelsize, int bits_per_pixel, PNeoEnv env)
{
  ...
  if (pixelsize == 1) {
    kl_average_plane << <blocks, threads >> >((uchar4*)srcp, (const uchar4*)otherp,
      src_pitch4, other_pitch4, width4, src_height);
    DEBUG_SYNC;
  }
  else if (pixelsize == 2) {
    kl_average_plane << <blocks, threads >> >((ushort4*)srcp, (const ushort4*)otherp,
      src_pitch4, other_pitch4, width4, src_height);
    DEBUG_SYNC;
  }
  else { // pixelsize == 4
    kl_average_plane << <blocks, threads >> >((float4*)srcp, (const float4*)otherp,
      src_pitch4, other_pitch4, width4, src_height);
    DEBUG_SYNC;
  }
}
...
static void merge_plane(BYTE* srcp, const BYTE* otherp, int src_pitch, int other_pitch, int src_rowsize, int src_height, float weight, int pixelsize, int bits_per_pixel, PNeoEnv env)
{
  ...
  {
    if (IS_CUDA) {
      average_plane_cuda(srcp, otherp, src_pitch, other_pitch, src_rowsize, src_height, pixelsize, bits_per_pixel, env);
      return;
    }
  ...
  {
    if (IS_CUDA) {
      merge_plane_cuda(srcp, otherp, src_pitch, other_pitch, src_rowsize, src_height, weight, pixelsize, bits_per_pixel, env);
      return;
    }
    int weight_i;
    int invweight_i;
...
PVideoFrame __stdcall MergeChroma::GetFrame(int n, IScriptEnvironment* env_)
{
  ...
  if (weight<0.9961f) {
    if (vi.IsYUY2()) {
      if (IS_CUDA)
        env->ThrowError("MergeChroma: YUY2 on CUDA is not supported");
    ...
    if (vi.IsYUY2()) {
      ...
      if (src->IsWritable()) {
        src->GetWritePtr(PLANAR_Y); //Must be requested
        Copy(src->GetWritePtr(PLANAR_U),src->GetPitch(PLANAR_U),chroma->GetReadPtr(PLANAR_U),chroma->GetPitch(PLANAR_U),chroma->GetRowSize(PLANAR_U),chroma->GetHeight(PLANAR_U), env);
        Copy(src->GetWritePtr(PLANAR_V),src->GetPitch(PLANAR_V),chroma->GetReadPtr(PLANAR_V),chroma->GetPitch(PLANAR_V),chroma->GetRowSize(PLANAR_V),chroma->GetHeight(PLANAR_V), env);
        if(vi.IsYUVA())
          Copy(src->GetWritePtr(PLANAR_A),src->GetPitch(PLANAR_A),chroma->GetReadPtr(PLANAR_A),chroma->GetPitch(PLANAR_A),chroma->GetRowSize(PLANAR_A),chroma->GetHeight(PLANAR_A), env);
      }
      else { // avoid the cost of 2 chroma blits
        PVideoFrame dst = env->NewVideoFrame(vi);
        
        Copy(dst->GetWritePtr(PLANAR_Y),dst->GetPitch(PLANAR_Y),src->GetReadPtr(PLANAR_Y),src->GetPitch(PLANAR_Y),src->GetRowSize(PLANAR_Y),src->GetHeight(PLANAR_Y), env);
        Copy(dst->GetWritePtr(PLANAR_U),dst->GetPitch(PLANAR_U),chroma->GetReadPtr(PLANAR_U),chroma->GetPitch(PLANAR_U),chroma->GetRowSize(PLANAR_U),chroma->GetHeight(PLANAR_U), env);
        Copy(dst->GetWritePtr(PLANAR_V),dst->GetPitch(PLANAR_V),chroma->GetReadPtr(PLANAR_V),chroma->GetPitch(PLANAR_V),chroma->GetRowSize(PLANAR_V),chroma->GetHeight(PLANAR_V), env);
        if(vi.IsYUVA())
          Copy(dst->GetWritePtr(PLANAR_A),dst->GetPitch(PLANAR_A),chroma->GetReadPtr(PLANAR_A),chroma->GetPitch(PLANAR_A),chroma->GetRowSize(PLANAR_A),chroma->GetHeight(PLANAR_A), env);
...
int __stdcall MergeChroma::SetCacheHints(int cachehints, int frame_range) {
  switch (cachehints) {
  ...
  if (weight>0.9961f) {
    ...
    if (luma->IsWritable() && vi.IsSameColorspace(vi2)) {
      if (luma->GetRowSize(PLANAR_U)) {
        ...
        Copy(luma->GetWritePtr(PLANAR_U),luma->GetPitch(PLANAR_U),src->GetReadPtr(PLANAR_U),src->GetPitch(PLANAR_U),src->GetRowSize(PLANAR_U),src->GetHeight(PLANAR_U), env);
        Copy(luma->GetWritePtr(PLANAR_V),luma->GetPitch(PLANAR_V),src->GetReadPtr(PLANAR_V),src->GetPitch(PLANAR_V),src->GetRowSize(PLANAR_V),src->GetHeight(PLANAR_V), env);
      }
      if (luma->GetPitch(PLANAR_A)) // copy Alpha if exists
        Copy(luma->GetWritePtr(PLANAR_A),luma->GetPitch(PLANAR_A),src->GetReadPtr(PLANAR_A),src->GetPitch(PLANAR_A),src->GetRowSize(PLANAR_A),src->GetHeight(PLANAR_A), env);
      ...
      Copy(dst->GetWritePtr(PLANAR_Y),dst->GetPitch(PLANAR_Y),luma->GetReadPtr(PLANAR_Y),luma->GetPitch(PLANAR_Y),luma->GetRowSize(PLANAR_Y),luma->GetHeight(PLANAR_Y), env);
      if (src->GetRowSize(PLANAR_U) && dst->GetRowSize(PLANAR_U)) {
        Copy(dst->GetWritePtr(PLANAR_U),dst->GetPitch(PLANAR_U),src->GetReadPtr(PLANAR_U),src->GetPitch(PLANAR_U),src->GetRowSize(PLANAR_U),src->GetHeight(PLANAR_U), env);
        Copy(dst->GetWritePtr(PLANAR_V),dst->GetPitch(PLANAR_V),src->GetReadPtr(PLANAR_V),src->GetPitch(PLANAR_V),src->GetRowSize(PLANAR_V),src->GetHeight(PLANAR_V), env);
      }
      if (dst->GetPitch(PLANAR_A) && src->GetPitch(PLANAR_A)) // copy Alpha if in both clip exists
        Copy(dst->GetWritePtr(PLANAR_A),dst->GetPitch(PLANAR_A),src->GetReadPtr(PLANAR_A),src->GetPitch(PLANAR_A),src->GetRowSize(PLANAR_A),src->GetHeight(PLANAR_A), env);
...
int __stdcall MergeLuma::SetCacheHints(int cachehints, int frame_range) {
  switch (cachehints) {
  case CACHE_GET_MTMODE:
    return MT_NICE_FILTER;
  case CACHE_GET_DEV_TYPE:
...
int __stdcall MergeAll::SetCacheHints(int cachehints, int frame_range) {
  switch (cachehints) {
  case CACHE_GET_MTMODE:
    return MT_NICE_FILTER;
  case CACHE_GET_DEV_TYPE:
